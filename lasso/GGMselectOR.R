...
selectFastOR <- function(X,
  ...
  # Main calculations
  return( calcLarsNEWOR(X, XNorm, Dmax,  pen, family,
                      min.ev, max.iter, eps,
                      beta, tau,h, T0,
                      verbose))
}
...
calcLarsNEWOR <- function(X, XNorm, Dmax, pen, family,
                        min.ev,
                        max.iter, eps,
                        beta, tau, h,  T0,
                        verbose) {
  ...
  for (afamily in family)
    {
      ...
      if ( (afamily == "LA") || (afamily == "EW"))  {
        Gr[,]  <- 0
        NVoisGr[] <- 0
        GrGlob  <- calcModLassoOR(XNorm, Dmax, afamily, max.st,
...
as.integer(NVoisGraph),  as.integer(Graph),
  ...
  if (is.element("C01",family) && is.element("LA",family)) {
    ...
    # critère parmi celui calcule par LA et celui calcule par C01
    ...
    for (iK in 1:lK) {
      ...
      switch(ind.min[iK],
             # =1 (le 1ier critère est celui calcule par LA)
             output$C01.LA$Neighb[,,iK] <- output$LA$Neighb[,,iK],
             # =2 (le 2ième critère est celui calcule par C01)
