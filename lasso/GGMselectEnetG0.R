# Version de selectFast "lasso" permettant d inclure un graphe G0
...
selectFast <- function(X,
  ...
  # INPUT
  # Entrees identiques a celles de selectQE:
  ...
  #   G0 : intial graph: p x p adjacency matrix, symmetric with 0 on the diagonal
  #   gamma : >0 parameter of l2 constraint
  # Other:
  ...
  # pen is max(Dmax+1) x lK x p
  
  # Main calculations
  return( calcLarsNEW(X, XNorm, Dmax,  pen, family,
                      min.ev, max.iter, eps,
                      beta, tau,h, T0, G0, d0, gamma,gamma0,
                      verbose))
}
...
calcLarsNEW <- function(X, XNorm, Dmax, pen, family,
  ...
  #   d0 : p-dimensional vertor of integers
  ...
  for (afamily in family)
    {
      ...
      if ( (afamily == "LA") || (afamily == "EW"))  {
        ...
        GrGlob  <- calcModLasso(XNorm, Dmax, afamily, max.st,
                                max.iter, eps, beta, tau,h, T0,G0,d0,gamma,gamma0) #G0
      } 
      switch(afamily,
             LA =
             {
...
dimnames(res$Neighb) <- dimnames(Neighb)
             ...
             C01 =
             { stop("Sorry, C01 is not allowed with G0") #G0
