...
# incase no csf above the mask. just create a fake one.
if [[ -s ${nuisance_dir}/csf.1D ]]
then
    echo "csf.1D is normal."
else
    echo "csf.1D is empty. will create a fake file with all one."
    for i in {1..240}; do printf "1\n" >> ${nuisance_dir}/csf.1D; done;
fi
