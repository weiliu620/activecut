...
int main(int argc, char* argv[])
{
     ...
     mydesc.add_options()
	  ...
	  ("snapshotfreq", po::value<unsigned>(&par.snapshotfreq)->default_value(500),
	   "number of scans for burn-in period. ")
	     ...
	     ParStruct & par)
{
     ...
     for (unsigned scanIdx = 0; scanIdx < par.burnin + par.numSamples; scanIdx ++) {
	  ...
	  if (scanIdx%par.snapshotfreq == 0 ) {
	       SaveRunningSamples2(theGraph, coordMap, rSampleMap, par, scanIdx);
	  }
		...
		ParStruct & par)
{
     ...
     do {
	  numIter ++;
	  beta_o = par.beta;
	  priorEngy = CompBetaDrv(theGraph, coordMap, rSampleMap, edgeMap, par, drv1, drv2);
