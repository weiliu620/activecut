...
int main(int argc, char* argv[])
{
     ...
     for (boost::filesystem::directory_iterator fmriPathIt(fmriPathVar); fmriPathIt != fmriPathEnd; ++fmriPathIt) {
     	  ...
     	  cout <<  "initmrf(): add " << (*fmriPathIt).path().string() << "\n";
     }
     ...
     grpPtr->SetOrigin( maskPtr->GetOrigin() );
     grpPtr->SetSpacing( maskPtr->GetSpacing() ); 
     grpPtr->SetDirection( maskPtr->GetDirection () );
     ...
     // Define a bestvmm to save best vmm.
     std::vector <VMM> bestvmm(numSubs);
     ...
     for (repeatIdx = 0; repeatIdx < kmeansIter; repeatIdx ++) {
	  ...
	  if (verbose >= 1) {
	       ...
	       save3dcharInc(grpPtr, grouplabel);
     	  }
     }
