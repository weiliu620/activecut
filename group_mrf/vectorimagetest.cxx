...
int main( int, char* argv[] )
{
...

     {
	  ...
	  typedef itk::Image< vnl_vector< PixelType >, Dimension > VnlVectorImageType;
	  ...
	  // Using image of vnl_vector<pixelType>
	  {
	       typedef vnl_vector< PixelType > InternalPixelType;

	       itk::TimeProbe clock;
	       clock.Start();

	       VnlVectorImageType::Pointer image = VnlVectorImageType::New();
	       VnlVectorImageType::IndexType start;
	       InternalPixelType f( VectorLength , 0);
	       VnlVectorImageType::SizeType  size;
	       for( unsigned int i=0; i<VectorLength; i++ ) { f[i] = i; }
	       start.Fill(0);
	       size.Fill(50);
	       VariableLengthVectorImageType::RegionType region;
	       ...
	       {
		    clock.Start();
		    typedef itk::ImageRegionConstIterator< VnlVectorImageType > IteratorType;
		    IteratorType it( image, image->GetBufferedRegion() );
		    it.Begin();
		    while( !it.IsAtEnd() )
		    {
			 f = it.Get();
			 for( unsigned int i=0; i<VectorLength; i++ ) { printf("%2.f ", f[i]); };
			 printf("\n");
			 ++it;
		    }
		    clock.Stop();
