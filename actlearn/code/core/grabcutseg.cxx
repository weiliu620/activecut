...
int main(int argc, char* argv[])
{
     ...
     load_constraints(lindexPtr, initPtr, hard_constraints);
     ...
     // NxK matrix to save the GMM class labels of both FG and BG voxels. K is
     // the max of the FG and BG's number of clusters.
     ...
     bool LL_changed = true; // log-likelihood changes a lot?
     unsigned em_iter = 0;
     ...
     while( (LL_changed ) && em_iter < maxem)
     {
	  ...
	  // M step. Estimate parameters. M step need to be in front of E
	  // step, as at this stage, we know labels from K-Means, but not
	  // the GMM parameters.
	  ...
	  // E step: Given GMM parameters, estimate expectation of labels.
	  ...
	  // evaluate log-likelihood to decide if we need to stop.
     ...
     logistic_init(score_map, data, con_map, alpha, par, par.eta/4, 1); 
     logistic_init(score_map, data, con_map, alpha, par, par.eta/2, 1); 
     ...
     save_score(score_map, lindexPtr, score_file);
     return 0;
}
