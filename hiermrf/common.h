...
struct GMMComp {
     float mu;
     float sigma;
     unsigned numPts; // # of data points, including all Monte Carlo samples
     float prop;
};
...
struct SWParType {
     ...
     double gamma;
     unsigned short verbose;
     unsigned totalPts; // # of all data points, not counting MC samples.
};
...
struct ArgList {
     SWParType par;
     unsigned numThreads;
     unsigned numPtsPerThreads;
     lemon::ListGraph * theGraph;
     lemon::ListGraph::NodeMap< unsigned char > * labelMap;
     lemon::ListGraph::EdgeMap<bool > * edgeMap;
};
