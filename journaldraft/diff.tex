...
%DIF DEL revison1.tex       Tue Apr 29 20:38:32 2014
%DIF ADD journaldraft.tex   Wed May 28 14:22:36 2014
%DIF 2d2
%DIF < 
%DIF -------
...
\begin{figure*}[htb]
  ...
  \caption{Comparison of segmentation methods for group study of \DIFdelbeginFL \DIFdelFL{fc-fMRI}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{rs-fMRI}\DIFaddendFL . Most
...
constraint~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{ng2012modeling, varoquaux2011multi, beckmann2009group}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{beckmann2009group, varoquaux2011multi, ng2012modeling,
  ng2012group}
}%DIFAUXCMD
}\DIFaddend . An accurate estimate of an individual's network is an important
...
unreliable subject maps \DIFdelbegin \DIFdel{is also }\DIFdelend \DIFaddbegin \DIFadd{might be }\DIFaddend unreliable. In other
...
these time courses~\citep{filippini2009distinct,beckmann2009group}. \DIFdelbegin \DIFdel{Such methods
treat voxel intensity from all subjects the same way as for group map
estimation, ignoring that they may have }\DIFdelend \DIFaddbegin \DIFadd{The
}\DIFaddend subject-specific \DIFdelbegin \DIFdel{variances.
}\DIFdelend \DIFaddbegin \DIFadd{spatial maps enable hypothesis testing to determine the voxels
that are statistically significant within each network.
%DIF > % Such methods treat voxel intensity from all subjects the same way as for group
%DIF > % map estimation, ignoring that they may have subject-specific variances. 
}\DIFaddend Both classes of approaches do not refine the initial group or subject estimates,
...
mixture representation\DIFaddbegin \DIFadd{, and the multilevel framework is similar to GLM in
...
solution~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{descombes1998spatio, ou2010combining, ng2012group}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{descombes1998spatio, ou2010combining, ng2012modeling}
}%DIFAUXCMD
}\DIFaddend . Depending on
...
linear model (GLM)}\DIFdelend \DIFaddbegin \DIFadd{GLM}\DIFaddend ~\citep{penny2005bayesian}, on the parameters of a spatio-temporal
...
we define MRF on the latent network label variables of \DIFaddbegin \DIFadd{a }\DIFaddend hidden Markov model
...
similar patterns with \DIFdelbegin \DIFdel{another subject's}\DIFdelend \DIFaddbegin \DIFadd{those of another subject}\DIFaddend , implicitly represented by the
...
% Other related work.
The hierarchical concept has been explored in the \DIFdelbegin \DIFdel{class of ICA-based methods }\DIFdelend \DIFaddbegin \DIFadd{signal decomposition framework
}\DIFaddend by \citet{varoquaux2010group, varoquaux2011multi}. The authors of both works
...
\citet{varoquaux2010group}
}%DIFAUXCMD
identify a subspace of reproducible components across
...
subject-specific spatial maps. In \mbox{%DIFAUXCMD
\citet{varoquaux2011multi}
}%DIFAUXCMD
, the mixing
matrices of the }\DIFaddend group and subject level \DIFdelbegin \DIFdel{'s mixing matrices }\DIFdelend are solved jointly as a convex
optimization problem\DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\citep{varoquaux2011multi}
}%DIFAUXCMD
}\DIFdelend . With regard to the hierarchical concept and the joint
...
In the remainder of the paper, we \DIFdelbegin \DIFdel{give some background information and related
works in section \ref{sec:ref}, define the }\DIFdelend \DIFaddbegin \DIFadd{define the graphical }\DIFaddend model in section
\ref{sec:model}, and give the approximate inference procedure in section
\ref{sec:inference}. \DIFdelbegin \DIFdel{We
compare }\DIFdelend \DIFaddbegin \DIFadd{The comparisons of }\DIFaddend the accuracy and consistency of \DIFdelbegin \DIFdel{our method , together }\DIFdelend \DIFaddbegin \DIFadd{the
proposed method }\DIFaddend with other methods \DIFdelbegin \DIFdel{by testing }\DIFdelend on synthetic and real data \DIFdelbegin \DIFdel{experiments }\DIFdelend \DIFaddbegin \DIFadd{are given }\DIFaddend in
section \ref{sec:synexperiments} and section \ref{sec:realexperiments}\DIFdelbegin \DIFdel{and discuss the model and algorithm performance}\DIFdelend \DIFaddbegin \DIFadd{. We
discuss the algorithm performance, relation to other models, and some caveats }\DIFaddend in
...
proposed energy function represents both priors\DIFdelbegin \DIFdel{without enforcing the smoothness
by altering the original BOLD signal as Gaussian spatial filer usually does. }\DIFdelend \DIFaddbegin \DIFadd{, mitigating the possible
...
strength.
 ...
 %DIF > % without enforcing the smoothness usually introduced by altering the original
 %DIF > % BOLD signal in Gaussian spatial filer.

\DIFaddend As for the inference, although appearing different in the image domain,
...
toolbox\DIFdelbegin \DIFdel{~}\DIFdelend \footnote{http://mialab.mrn.org/software/}\DIFdelbegin \DIFdel{to estimate the spatial
component maps }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend with number of component \DIFdelbegin \DIFdel{also set
to seven}\DIFdelend \DIFaddbegin \DIFadd{set
to 7 and 25 respectively}\DIFaddend , and convert the overlapped spatial maps to discrete
labels by selecting the component with the strongest signal at each voxel. The
discretization of ICA map may \DIFdelbegin \DIFdel{result in lost
of }\DIFdelend \DIFaddbegin \DIFadd{lose }\DIFaddend information compared to Zuo et al.'s
...
K-Means multiple times and have chosen the best solutions. \DIFaddbegin \DIFadd{On the group level,
...
\begin{figure*}[htb]
  ...
  \caption{The intersession variance maps estimated by four methods. \DIFaddbeginFL \DIFaddFL{ICA-7 and
    ICA-25 denote ICA with 7 and 25 components, respectively. }\DIFaddendFL The variance maps
...
given \emph{a priori}\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, as }\DIFaddend the widely-accepted method of estimating this number
is not available. For 7\DIFaddbegin \DIFadd{, }\DIFaddend or even 17 functional networks suggested by
...
needed to identify those \emph{background} voxels. \DIFdelbegin \DIFdel{The key difference with
}\DIFdelend \DIFaddbegin \DIFadd{Similar to }\DIFaddend ICA-based
methods\DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend each voxel is assumed to belong to a single functional network. While a
large number of brain regions appear to have single functional pattern, studies
show that some regions, such as the precuneus, \DIFdelbegin \DIFdel{has }\DIFdelend \DIFaddbegin \DIFadd{have }\DIFaddend a wide spectrum of highly
integrated networks, including visuo-spatial imagery, episodic memory and
self-processing operation~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{cavanna2006precuneus}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{cavanna2006precuneus}
}%DIFAUXCMD
}\DIFaddend . These voxels with
overlapping functional patterns remain a challenge to segmentation-based methods
including HMRF, \DIFdelbegin \DIFdel{though }\DIFdelend \DIFaddbegin \DIFadd{despite that }\DIFaddend the posterior distribution \DIFdelbegin \DIFdel{helping }\DIFdelend \DIFaddbegin \DIFadd{helps }\DIFaddend the estimation to
some extent. Last, our model \DIFdelbegin \DIFdel{currently accounts for the moderate }\DIFdelend \DIFaddbegin \DIFadd{moderately accounts for }\DIFaddend intersubject variation. So,
it \DIFaddbegin \DIFadd{may }\DIFaddend have difficulty dealing with \DIFdelbegin \DIFdel{the
subject }\DIFdelend \DIFaddbegin \DIFadd{subjects }\DIFaddend whose functional network topology
is fundamentally different from the group average, for example, \DIFdelbegin \DIFdel{subject with }\DIFdelend \DIFaddbegin \DIFadd{a subject who
has had }\DIFaddend neurosurgery. 

\DIFaddbegin \DIFadd{On the experimental side, we showed the increased consistency of the HMRF model in
...
\DIFaddend % revision: phenotype relationship. 
...
supplementary material \DIFdelbegin \DIFdel{shows }\DIFdelend \DIFaddbegin \DIFadd{show }\DIFaddend that the percentage change \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend number of voxels
...
significant voxels \DIFdelbegin \DIFdel{of HMRF is due to that }\DIFdelend \DIFaddbegin \DIFadd{using HMRF is because }\DIFaddend the subject network maps shift towards
the population network map. The increased number of voxels for certain networks,
is because \DIFdelbegin \DIFdel{of the }\DIFdelend new correlations emerge after the HMRF model recovers more
accurate network maps. \DIFdelbegin \DIFdel{Overall, the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend small percentage of changed voxels indicates
the limited impact of HMRF on the possible phenotype correlation, though more
\DIFdelbegin \DIFdel{thorough experiments are needed after this preliminary test}\DIFdelend \DIFaddbegin \DIFadd{sophisticated approaches, such as \mbox{%DIFAUXCMD
\citet{alexander2012discovery}
}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet{reiss2012paradoxical}
}%DIFAUXCMD
, will be needed to elucidate whether HMRF model
illuminate or diminish the network-phenotype relations}\DIFaddend . 
...
imaging data exchange project. \DIFdelbegin \DIFdel{These projects have even greater
heterogeneity of scanning parameters than single-site data because of }\DIFdelend \DIFaddbegin \DIFadd{Because of a }\DIFaddend wider range of subject groups with
different pathologies~\citep{smith2012future}, \DIFdelbegin \DIFdel{despite the efforts of }\DIFdelend \DIFaddbegin \DIFadd{the data in these projects has
even greater heterogeneity of scanning parameters than single-site data, despite
efforts toward }\DIFaddend strict quality control process~\citep{marcus2013human}. Our model
fits the hierarchical site-subject nature of the data acquisition process and is
able \DIFdelbegin \DIFdel{take into account }\DIFdelend \DIFaddbegin \DIFadd{to account for }\DIFaddend the heterogeneity in the data. \DIFdelbegin \DIFdel{Accordingly, with }\DIFdelend \DIFaddbegin \DIFadd{With }\DIFaddend more reliable estimation
of the functional network patterns as imaging phenotype, our model may \DIFdelbegin \DIFdel{helps revealing
}\DIFdelend \DIFaddbegin \DIFadd{also
reveal }\DIFaddend more genotype-phenotype interaction at the subject level \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{by }\DIFaddend using large,
lower-quality \DIFdelbegin \DIFdel{dataset}\DIFdelend \DIFaddbegin \DIFadd{datasets. Equally encouraging is the application of our model
