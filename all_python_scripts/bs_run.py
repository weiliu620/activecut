...
def bs_run(bs_start, bs_end, bs_dir):
    ...
    fmri_smooth_nii = os.path.join(bs_dir, 'fmri_smooth_nii') 
    ...
    if not os.path.exists(fmri_smooth_nii):
        os.makedirs(fmri_smooth_nii)
        


    for bsid in range(bs_start, bs_end + 1):
        print('----->>>> Boostrap {0} begin: <<<-----------'.format(bsid) )
        
        # resampling and normalizaion.
        # resample(session_smooth, fmri_smooth, bsid)
        # resample(session_little, fmri_little, bsid)

        # Clone a nii file from fmri_smooth, just for N-Cuts (pain!)
        # shutil.rmtree(fmri_smooth_nii)
        # shutil.copytree(fmri_smooth, fmri_smooth_nii)
        # all_subjects_files = os.listdir(fmri_smooth_nii)
        # gunzip_proc_set = set()
        # for sub_file in all_subjects_files:
        #     gunzip_proc_set.add(subprocess.Popen(['gunzip', os.path.join(fmri_smooth_nii, sub_file)]) )

        # for p in gunzip_proc_set:
        #     if p.poll() is None:
        #         p.wait()


        one_run(fmri_smooth, fmri_smooth_nii, fmri_little, bsid, bs_dir)
...
def one_run(fmri_smooth, fmri_smooth_nii, fmri_little, bsid, bs_dir):
    ...
    mask_file_nii = "/usr/sci/scratch/weiliu/NYU_test_retest/bootstrap/Yeo2011_7Networks_MNI152_FreeSurferConformed3mm.nii"
    ...
    #     proc_set.add(subprocess.Popen(['/home/sci/weiliu/projects/group_mrf/build_' + host_name + '/subkmeans', '--kmeansiter', '10', '--seed', '1', '--numCluster', '7', '-v', '0', '--fmri', os.path.join(fmri_smooth, fmri_file), '--mask', mask_file, '--labelmap', os.path.join('kmeans', out_name)] ) )


    # # group kmeans
    # grp_full = os.path.join('kmeans', 'grp' + bsfix + '.nii.gz')

    # proc_set.add( subprocess.Popen(['/home/sci/weiliu/projects/group_mrf/build_'  + host_name + '/initmrf', '--fmripath', fmri_smooth, '-m', mask_file, '--kmeansiter', '10', '--numClusters', '7', '--seed', '0', '--outgrouplabel', grp_full]) )


    # N-Cuts
    allsubfiles = os.listdir(fmri_smooth_nii)
    ncuts_proc = set()
    for fmri_file in allsubfiles:
        sub_name = os.path.splitext(fmri_file)[0] 
        ncuts_sub_cmd =  'matlab -nodesktop  -nosplash -nojvm -r "addpath ~/projects/ncuts_fmri; dbstop if error; nyu_segts(\'' + os.path.join(fmri_smooth_nii, fmri_file) + '\', \'' + mask_file_nii + '\', 7, 0.4, \'' + os.path.join(bs_ncuts, fmri_file) + '\',\'' + os.path.join(bs_ncuts, sub_name+'.mat') + '\'); quit"' 
        # ncuts_sub_cmd =  'matlab -nodesktop  -nosplash -nojvm -r "addpath ~/projects/ncuts_fmri; dbstop if error; quit"' 
    ...
    # ncuts_grp_cmd =  'matlab -nodesktop  -nosplash -nojvm -r "addpath ~/projects/ncuts_fmri; dbstop if error; nyu_groupcuts(\'' + bs_ncuts + '\', 0.4, \'' + mask_file_nii + '\', 7, \'' + os.path.join(bs_ncuts, 'grp.nii') + '\'); quit"'
    ncuts_grp_cmd =  'matlab -nodesktop  -nosplash -nojvm -r "addpath ~/projects/ncuts_fmri; dbstop if error; quit"' 
    ...
    # all_kmeans_maps = os.listdir('kmeans')
    ...
    all_ncuts_files = [ f for f in os.listdir(bs_ncuts) if fnmatch.fnmatch(f, '*.nii') ]
